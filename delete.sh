...
remove() {
  ...
  if "$ask_func" "Remove '$pathname'?"; then
    rm -rf "$pathname"
  else
    return 2
  fi
...
heading "1/5. Remove entry point from '~/.zshenv' file"
  ...
  if [ "$?" -eq 0 ]; then
    if ask_yes "Remove lines from '$HOME/.zshenv':\n$grepped_line"; then
...
heading "2/5. Remove configuration folder"
  ...
  if [ -e "$pathname" ]; then
    remove ask_yes "$pathname" && print_done || print_skip
  else
    print_no_tasks
  fi
:
...
heading "3/5. Remove history"
  ...
  if [ -e "$pathname" ]; then
    remove ask_no "$pathname" && print_done || print_skip
  else
    print_no_tasks
  fi
:
...
heading "5/5. Remove cache files"
  ...
  for pathname in \
    ...
    remove ask_yes "$pathname" && [ ! "$?" -eq 1 ] && is_no_tasks=false
  done

  if [ "$is_no_tasks" = false ]; then
    print_done
  else
    print_no_tasks
  fi
