...
```sh #link ~/.config/zsh/zshrc
65: (( $+commands[keychain] )) && eval "$(keychain --eval --quiet --confhost --timeout 60)"
```
...
```sh #link ~/.config/zsh/zalias.zsh
22: alias v='nvim'
23: for v in 'vi' 'vm' 'vim'; do alias $v='v'; done
24: alias svim='sudo -Es nvim'
25: alias sv='svim'
26: alias lvim='nvim -c "lua require(\"persistence\").load()"'
27: alias lv='lvim'
```
...
```sh #link ~/.config/zsh/zshrc
69: (( $+commands[zoxide] )) && eval "$(zoxide init zsh)"
```
...
```sh #link ~/.config/zsh/zshrc
68: (( $+commands[gh] )) && eval "$(gh completion -s zsh)"
```
...
```sh #link ~/.config/zsh/zshrc
63: path+=("$HOME/.local/share/fnm")
...
67: (( $+commands[fnm] )) && eval "$(fnm env --use-on-cd)"
```
