...
```sh #link ~/.config/zsh/zshrc
60: (( $+commands[keychain] )) && eval "$(keychain --eval --quiet --confhost --timeout 60)"
```
...
```sh #link ~/.config/zsh/zshrc
64: (( $+commands[zoxide] )) && eval "$(zoxide init zsh)"
```
...
```sh #link ~/.config/zsh/zshrc
63: (( $+commands[gh] )) && eval "$(gh completion -s zsh)"
```
...
...
62: (( $+commands[fnm] )) && eval "$(fnm env --use-on-cd)"
```
