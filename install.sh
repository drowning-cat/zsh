...
if ! grep "^$USER:" /etc/passwd | grep -q "/bin/zsh"; then
  heading "0. Init zsh shell"
    if ask_yes "Change current user's login shell to 'zsh'?"; then
      chsh -s `which zsh`
...
heading "2/5. Copying configuration files"
  if [ ! -e "$ZSH_CONFIG_FOLDER" ] || ask_yes "Replace '$ZSH_CONFIG_FOLDER'"; then
...
heading "4/5. Installing '$FONT_NAME' font"
  ...
  if [ ! -e "$font_src" ] ||
     [ ! -e "$font_dst" ] || ask_yes "Replace '$font_dst'?"; then
    mkdir -p "$font_dst"
    cp -Rv "$font_src" "$_"
    br
